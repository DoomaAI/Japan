...
test('the games are offered easiest first, and can be narrowed to the Japanese ones',async()=>{
 ...
 assert.equal(rows.length,21,'every game is in the picker with a difficulty on it');
 ...
 for(const id of ['picross','gomoku','shogi','sumo'])
...
test('winning is celebrated once, and only where there is something to win',async()=>{
 ...
 for(const file of ['Karuta','AnimalShogi','Fukuwarai','Daruma','Shiritori','Picross','Gomoku','SpotDifference','Origami']){
