...
export default function TicketAttachments({ticket,attachments,members,user,enabled,busy,setBusy,request,accept,notice,onView,onEdit,onRemove,saveOffline}){
 ...
 return <div className="ticket-attachments">
  {!!attachments.length&&<><h3>Ticket photos & files</h3>{attachments.map(a=><div className="ticket-attachment" key={a.id}><button type="button" onClick={()=>onView(a)}>{a.type.startsWith('image/')&&<img src={`/api/document?id=${a.id}`} alt="" loading="lazy"/>}<span><strong>{a.person}</strong><small>{a.title}</small></span></button><div className="row wrap"><button onClick={()=>saveOffline(`/api/document?id=${a.id}`,`doc-${a.id}`)}>Save offline</button>{user.role==='parent'&&<><button onClick={()=>onEdit(a)}>Edit person / tags</button><button className="danger" disabled={busy} onClick={()=>onRemove(a)}>Remove file</button></>}</div></div>)}</>}
