...
import {ASK_LIMIT,askDayLabel,askHistory,askStarters,readThread,stepStarters,writeThread} from './ask-thread.js';
...
export default function AskTrip({state,user,day,step,config,online=true,request,go,selectDay,notice}){
 const [all,setThread]=useState(()=>readThread(user?.name));
 const thread=step?all.filter(item=>item.step===step.id):all;
 ...
 const clear=()=>keep(step?all.filter(item=>item.step!==step.id):[]);
 async function ask(text){
  ...
  try{
   ...
   keep([{id:`${Date.now()}`,at:new Date().toISOString(),...answer},...all]);
   setQuestion('');
 ...
 return <div className="ask">
  ...
  {thread.map(item=><article className="feature-card ask-card" key={item.id}>
   ...
   <small>{item.step&&!step?`About ${state.steps.find(s=>s.id===item.step)?.title||'a stop'} · `:''}{item.about?`About ${askDayLabel(item.about)}`:'About the whole trip'} · {item.usage?.searches||0} web {item.usage?.searches===1?'search':'searches'} · nothing was changed</small>
  </article>)}
  ...
  {!!thread.length&&<div className="row wrap"><button onClick={()=>{clear();notice?.(step?'Your questions about this stop are cleared.':'Your questions on this phone are cleared.');}}><Trash2 size={16}/>{step?'Clear these questions':'Clear my questions'}</button>
