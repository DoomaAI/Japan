...
export function FactOfDay({queue,dateLabel,busy,dismiss,openPage,young}){
 ...
 return <div className="fact-of-day"
  ...
  {canRead&&<ReadAloudButton id={fact.id} text={factAloud(fact)} reading={reading} read={read} what="fact" rate={young?YOUNG_RATE:undefined} className={young?'young':''}/>}
...
export default function FunFacts({state,user,day,mutate,busy,openPage}){
 ...
 return <>
  ...
  <section className="my-phrases">
   ...
   {extra&&<div className="phrase-extra"><p className="eyebrow">ONE MORE</p><FactRow fact={extra} aloud={aloud}/></div>}
  ...
  {found?<section className="fact-section">
   ...
   {found.map(f=><FactRow key={f.id} fact={f} openPage={openPage} aloud={aloud}/>)}
   ...
   {!!todays.length&&<section className="fact-section">
    ...
    {todays.map(f=><FactRow key={f.id} fact={f} openPage={openPage} aloud={aloud}/>)}
   </section>}
...
export function FactRow({fact,openPage,aloud}){
 return <article className="fact-row">
  ...
  <div>
   ...
   <p>{fact.text}</p>
   <div className="row wrap fact-actions">
    {aloud&&<ReadAloudButton id={`row-${fact.id}`} text={factAloud(fact)} reading={aloud.reading} read={aloud.read} what="fact" rate={aloud.rate}/>}
