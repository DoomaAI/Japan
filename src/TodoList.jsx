...
import {ListChecks,ShoppingBag,Plus,Trash2,CalendarDays,ChevronRight,Inbox} from 'lucide-react';
import {dayLabel} from './AdventurePages.jsx';
import {TODO_KINDS,todos,todosFor,todoProgress,unallocatedTodos} from './trip-features.js';
...
function TodoRow({item,user,mutate,busy,onEdit,showDay}){
...
export function DayTodos({state,user,day,mutate,busy,go}){
 ...
 return <section className="day-todos" aria-label="Things to do or buy on this day">
  ...
  {list.map(item=><TodoRow key={item.id} item={item} user={user} mutate={mutate} busy={busy}/>)}
...
export default function TodoList({state,user,mutate,busy,go}){
 ...
 {!!loose.length&&<section className="todo-group">
  ...
  {loose.map(item=><TodoRow key={item.id} item={item} user={user} mutate={mutate} busy={busy} onEdit={setEdit}/>)}
 </section>}
 {byDay.map(({day:d,list})=><section className="todo-group" key={d.date}>
  <h2>{dayLabel(d.date)} · {d.city}</h2>
  <p><small>{d.title}</small></p>
  {list.map(item=><TodoRow key={item.id} item={item} user={user} mutate={mutate} busy={busy} onEdit={setEdit} showDay={false}/>)}
 </section>)}
 ...
 {go&&<p className="callout"><ShoppingBag size={18}/>For a proper shop — budgets, quantities, which shop and a link — use the <button onClick={()=>go('shopping')}>Shopping list</button>. This one is for the small things you just need to remember.</p>}
 </>;
}
