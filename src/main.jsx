...
function App(){
 ...
 return <PhraseAudio.Provider value={{clips:visibleState?.phraseAudio||{},user,request,accept,notice,config,busy}}>
  ...
  {tab==='days'&&<>{/* The same strip of dates as Home, so the week reads the same way on both; a date here
   ...
   {dayStrip(d=>go('glance',d))}<p className="eyebrow">21 SEPTEMBER — 6 OCTOBER</p><h1>Our days in Japan</h1><Button icon={ImageIcon} onClick={()=>setModal({type:'media'})}>Family photo & video gallery</Button><div className="journey-cover"><img src="/cover.jpg" alt="Original first-page cover of the Pasfield family Japan Travel Guide 2026"/></div>{/* A day every one of us has already walked through does not need a hotel address and a city on
