...
function App(){
 ...
 const [guidePage,setGuidePage]=useState(Number(new URLSearchParams(location.search).get('page'))||1),[reading,setReading]=useState(false),[guideIndex,setGuideIndex]=useState([]),[query,setQuery]=useState(''),[saved,setSaved]=useState(stored('japan.saved',[])),[clockShortcut,setClockShortcut]=useState(localStorage.getItem('japan.shortcut')||'');
 ...
 const envRef=useRef(envelope),queueRef=useRef(queue),working=useRef(false),touch=useRef(null),guideFlip=useRef(null),noteShown=useRef(''),phraseSeen=useRef(''),factShown=useRef(''),landed=useRef(false);
 ...
 return <PhraseAudio.Provider value={{clips:visibleState?.phraseAudio||{},user,request,accept,notice,config,busy}}>
  ...
  {tab==='days'&&<><p className="eyebrow">21 SEPTEMBER — 6 OCTOBER</p><h1>Our days in Japan</h1><Button icon={ImageIcon} onClick={()=>setModal({type:'media'})}>Family photo & video gallery</Button><div className="journey-cover"><img src="/cover.jpg" alt="Original first-page cover of the Pasfield family Japan Travel Guide 2026"/></div>{/* A day every one of us has already walked through does not need a hotel address and a city on
    ...
    <div className="days-grid">{state.days.map((d,i)=>{const progress=dayProgress(visibleState,d.date);return <button className={`day-tile${progress.finished?' finished':''}`} key={d.date} onClick={()=>go('glance',d.date)}><div><span className="eyebrow">DAY {i+1} · {fmtDay(d.date)}</span><span className="tag">{progress.done}/{progress.steps}</span></div>{todoProgress(visibleState,d.date).open>0&&<span className="tag todo-tag"><ListChecks size={12}/>{todoProgress(visibleState,d.date).open} to do</span>}<h2>{d.title}</h2>{progress.finished?<small className="day-finished"><Check size={14}/>{progress.skipped?`Done · ${progress.skipped} skipped`:'All done'} · {d.city}</small>:<><p><MapPin size={15}/>{d.city}</p><small>{d.hotel}</small></>}<ChevronRight className="tile-arrow"/></button>;})}</div></>}
  ...
  <BottomNav tab={tab} user={user} go={go} prefs={navPrefs} unread={state.alerts.some(a=>!a.seenBy?.[user.name])}/>
