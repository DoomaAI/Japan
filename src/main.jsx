...
function App(){
 ...
 return <PhraseAudio.Provider value={{clips:visibleState?.phraseAudio||{},user,request,accept,notice,config,busy}}>
  ...
  {tab==='tickets'&&<><p className="eyebrow">ALL THE DETAILS, TOGETHER</p><h1>Tickets & reservations</h1>{parent&&<DocumentReader config={config} busy={busy} setBusy={setBusy} request={request} notice={notice} mutate={mutate}/>}<Tickets state={state} user={user} config={config} busy={busy} setBusy={setBusy} accept={accept} mutate={mutate} notice={notice} saved={saved} saveOffline={saveOffline} selectStep={selectStep}/></>}
...
function Tickets({state,user,step,initialSearch='',config,busy,setBusy,accept,mutate,notice,saved,saveOffline,selectStep}){
 ...
 const docs=state.documents.filter(d=>d.category!=='memory'&&!d.parentDocumentId&&(all||d.stepId===step?.id)&&(!categoryFilter||(d.category||'ticket')===categoryFilter)&&(!personFilter||d.person===personFilter||state.documents.some(a=>a.parentDocumentId===d.id&&a.person===personFilter))&&(!search||[d.title,d.reference,d.notes,...(d.tags||[]),...state.documents.filter(a=>a.parentDocumentId===d.id).flatMap(a=>[a.title,a.notes,...(a.tags||[])])].join(' ').toLowerCase().includes(search.toLowerCase()))),parent=user.role==='parent';
