...
import {Volume2,Square,SkipForward,RotateCcw,Sparkles,Camera,ShoppingBag} from 'lucide-react';
import MissionArt from './MissionArt.jsx';
import Dictate from './Dictate.jsx';
import {BOYS,yenPerAud,yenToAud,shortlistToShop} from './trip-features.js';
...
// before it could ever be typed, so talking is what this box is for and the keyboard is the
// fallback rather than the other way round. Saying it fills the box; the button still saves it,
// so nothing goes to the family until a child decides it is right.
function Discovery({mission,boy,mine,parent,busy,mutate,edit}){
 const box=useRef(null),done=!!mission.completions[boy];
 return <form onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);await mutate({type:'challengeStatus',id:mission.id,person:boy,done:true,response:f.get('response')});}}>
  <label>My discovery<textarea ref={box} name="response" maxLength={2000} defaultValue={mission.responses?.[boy]||''} placeholder="Say it out loud, type it, or tell a parent" disabled={!mine}/></label>
  {mine&&<Dictate into={box} label="Say it" what="what you found"/>}
  <div className="row wrap"><button className="primary" disabled={busy||!mine}>{done?'Save my discovery':'I did it!'}</button>{done&&<button type="button" disabled={busy||!mine} onClick={()=>mutate({type:'challengeStatus',id:mission.id,person:boy,done:false})}>Undo</button>}{!done&&<button type="button" disabled={busy||!mine} onClick={()=>mutate({type:'challengeSkip',id:mission.id,person:boy,done:true})}><SkipForward size={16}/>Not this one</button>}{parent&&<button type="button" onClick={edit}>Edit mission</button>}</div>
 </form>;
}
...
export function Shopping({state,user,day,mutate,busy,initialId,go}){
 ...
 const waiting=shortlistToShop(state);
 ...
 return <><p className="eyebrow">GOOD FINDS, USEFUL THINGS</p><h1>Shopping list</h1><p>Souvenirs, gifts, snacks and things we need, with budgets, quantities and where to get them. For the small things you just need to remember, the To-do list is quicker.</p><button className="primary" onClick={()=>setEdit({person:user.name,day:null,quantity:1})}>Add an item</button><div className="document-filters"><label>Search<input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Item, shop or note"/></label><div className="form-row"><label>For<select value={person} onChange={e=>setPerson(e.target.value)}><option value="">Everyone</option>{['Family',...state.members].map(n=><option key={n}>{n}</option>)}</select></label><label>Status<select value={status} onChange={e=>setStatus(e.target.value)}><option value="needed">Still needed</option><option value="bought">Bought</option><option value="all">Everything</option></select></label><label>Day<DaySelect state={state} value={date} onChange={e=>setDate(e.target.value)} allowAll/></label></div></div><p><strong>{items.length} items</strong> · listed budgets ¥{items.reduce((sum,s)=>sum+(s.budget||0),0).toLocaleString()} <small>≈ ${yenToAud(items.reduce((sum,s)=>sum+(s.budget||0),0),yenPerAud(state)).toFixed(2)} at $1 = ¥{Math.round(yenPerAud(state))}</small> <small>Budget is the total for each item, including its quantity.</small></p><div className="feature-grid">{items.map(s=><article className={`feature-card ${s.boughtAt?'finished':''}`} key={s.id}><div className="section-heading"><h2>{s.title}</h2><input aria-label={`Mark ${s.title} bought`} type="checkbox" checked={!!s.boughtAt} disabled={busy} onChange={e=>mutate({type:'shoppingStatus',id:s.id,done:e.target.checked})}/></div><p>{s.quantity} × · {s.person}{s.budget!==null&&` · Budget ¥${s.budget.toLocaleString()} (≈$${yenToAud(s.budget,yenPerAud(state)).toFixed(2)})`}</p><small>{s.day?dayLabel(s.day):'Any day'}</small><p>{s.store}</p><p>{s.notes}</p>{s.boughtAt&&<small>Bought by {s.boughtBy} · {japanClock(new Date(s.boughtAt))} JST</small>}{s.shortlistId&&<small className="find-shopped"><Camera size={13}/> From the shortlist{go&&<> · <button className="linkish" onClick={()=>go('shortlist',null,s.shortlistId)}>see the photo</button></>}</small>}<div className="row wrap">{s.store&&<a target="_blank" rel="noreferrer" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(s.store+' Japan')}`}>Find shop</a>}{s.url&&<a href={s.url} target="_blank" rel="noreferrer">Website</a>}{parent&&<><button onClick={()=>setEdit(s)}>Edit</button><button className="danger" onClick={()=>{if(confirm('Remove this shopping item?'))mutate({type:'shoppingRemove',id:s.id});}}>Remove</button></>}</div></article>)}</div>{!items.length&&<div className="empty">Nothing on this list yet.</div>}
{waiting.length>0&&<section className="shortlist-waiting"><div className="section-heading"><div><p className="eyebrow">DECIDED ON, NOT YET ON THIS LIST</p><h2>{waiting.length} from the shortlist</h2></div>{go&&<button onClick={()=>go('shortlist')}>Shortlist</button>}</div><p><small>We said we are getting these. Bring one across and it arrives with its shop, its price and its notes, and stays linked to the photograph.</small></p>{waiting.map(f=><div className="list-row" key={f.id}><span>{f.title}<small>{[f.shop,f.place].filter(Boolean).join(' · ')||'No shop written down'}{f.price===null||f.price===undefined?'':` · ¥${f.price.toLocaleString()}`}</small></span><button disabled={busy} onClick={()=>mutate({type:'shortlistShop',id:f.id})}><ShoppingBag size={15}/>Put it on this list</button></div>)}</section>}
{go&&<p className="callout"><Camera size={18}/><span>Something we have seen in a shop and have not decided about goes on the <button onClick={()=>go('shortlist')}>Purchase shortlist</button>, with a photo of it and what the ticket said. This list is what we decided to buy before we left.</span></p>}
